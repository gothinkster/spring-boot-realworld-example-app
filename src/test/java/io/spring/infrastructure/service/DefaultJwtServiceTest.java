...
public class DefaultJwtServiceTest {
    ...
    public void should_generate_and_parse_token() {
        var user = new User("email@email.com", "username", "123", "", "");
        var token = jwtService.toToken(user);
        assertNotNull(token);
        var optional = jwtService.getSubFromToken(token);
    ...
    public void should_get_null_with_wrong_jwt() {
        var optional = jwtService.getSubFromToken("123");
    ...
    public void should_get_null_with_expired_jwt() {
        var token = "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJhaXNlbnNpeSIsImV4cCI6MTUwMjE2MTIwNH0.SJB-U60WzxLYNomqLo4G3v3LzFxJKuVrIud8D8Lz3-mgpo9pN1i7C8ikU_jQPJGm8HsC1CquGMI-rSuM7j6LDA";
