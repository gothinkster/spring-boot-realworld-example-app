...
public class MyBatisUserRepositoryTest extends DbTestBase {
    ...
    public void should_save_and_fetch_user_success() {
        ...
        var userOptional = userRepository.findByUsername("aisensiy");
        assertEquals(userOptional.get(), user);
        var userOptional2 = userRepository.findByEmail("aisensiy@163.com");
    ...
    public void should_create_new_user_follow_success() {
        var other = new User("other@example.com", "other", "123", "", "");
        userRepository.save(other);

        var followRelation = new FollowRelation(user.getId(), other.getId());
